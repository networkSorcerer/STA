		    ...
		    <form id="f_search" name="f_search">
		        ...
		        <div class="col-auto">
		            <select id="search" name="search" class="form-select form-select-sm" >
		                ...
		                <option value="animal_name">동물 ID</option>
		                <option value="animal_name">동물 이름</option>
						...
						<c:when test="${not empty adoptionList }">
							<c:forEach var="animal" items="${adoptionList }" varStatus="status">
								<tr class = "text-center id" data-num="${ adoption.adoptionId}">
									...
									<td class="animalid text-start">
									${adoption.animaId}
									</td>
									<td class="goDetail">${adoption.adoptionTitle }</td>
									<td class="name">${adoption.animalName }</td>
									<td class="status">${adoption.adoptionStatus }</td>
									<td class="regist">${adoption.adoptionDate}</td>
									<td class ="readcnt">${adoption.adoptionCnt }</td>
		...
		$(function(){
			...
			if(word!=""){
				...
				if($("#search").val()!='adoption_date'){
					...
					else if($("#search").val()=='animal_id') value="#list tr td.animalid";
					else if($("#search").val()=='animal_name')value="#list tr td.name"
					else if($("#search").val()=='adoption_status')value="#list tr td.status"
